document.addEventListener('DOMContentLoaded', () => {
    ...
    // El campo 'genero' se mantiene por si se usa en otro lado, pero la lógica de saludo ya no depende de él.
    // ***** INICIO DE LA MODIFICACIÓN 1: LISTA DE USUARIOS *****
    let users = JSON.parse(localStorage.getItem("users")) || [{
        ...
        'telefono': '',
        'genero': 'masculino'
    }, // Entrada para Mantenimiento con contraseña y.vera2025
    {
        'usuario': 'Yasbel Vera',
        ...
        'telefono': '',
        'genero': 'femenino'
    }, // Entrada para Checklist con contraseña Copesul2025
    {
        'usuario': 'Yasbel Vera',
    ...
    // NUEVO: Lista de nombres para saludo femenino.
    const usuariasFemeninas = ["Yasbel Vera", "Elisabeth Alcantara"];

    /**
     * Utiliza la API de Síntesis de Voz del navegador para decir un texto.
     * @param {string} texto - El texto que se va a reproducir.
     */
    ...
    // ***** INICIO DE LA MODIFICACIÓN 2: LÓGICA DE INICIO DE SESIÓN *****
    authForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const userValue = usernameInput.value.trim();
        const passValue = passwordInput.value.trim();

        // Se busca un usuario que coincida tanto en nombre como en contraseña.
        // atob() decodifica la contraseña desde Base64 para compararla.
        const usuarioEncontrado = users.find(u =>
            u.usuario.toLowerCase() === userValue.toLowerCase() &&
            atob(u.contraseña) === passValue
        );

        // Si se encuentra una combinación válida
        if (usuarioEncontrado) {
            loggedInUser = usuarioEncontrado.usuario;
            // Se guarda el rol (nivel) específico de la combinación que funcionó
            ...
            loadAllData();
            updateDashboardAccess(); // Esta función usará el rol correcto que se guardó
    ...
    const checklistItems = [{
        ...
        id: 'declaracion_jurada_domicilio',
        name: 'Declaración Jurada de Domicilio'
    }, {
        id: 'recibo_servicios',
        name: 'Recibo de Servicios (agua, luz, teléfono)'
    }, {
        id: 'contrato_trabajo',
        ...
        id: 'constancia_trabajo_anterior',
        name: 'Constancia de Trabajo Anterior'
    }, {
        id: 'certificado_estudios',
        name: 'Certificado de Estudios'
    }, {
        id: 'licencia_conducir',
        name: 'Licencia de Conducir (si aplica)'
    }, {
        id: 'record_conductor',
        name: 'Record de Conductor (si aplica)'
    }, {
        id: 'declaracion_jurada_salud',
        name: 'Declaración Jurada de Salud'
    }, {
        id: 'formato_cargos',
        name: 'Formato de Cargos'
    }, {
        id: 'ficha_datos_personales',
        name: 'Ficha de Datos Personales'
    }, {
    ...
    async function generateChecklistSummaryPdf(action = 'download') {
        try {
            ...
            doc.text(`COPESUL SAC - Generado por: ${user} el: ${new Date().toLocaleDateString('es-ES')}`, pageWidth / 2, 80, {
                align: 'center'
            });
            currentY = 110;
            ...
            doc.autoTable({
                startY: currentY,
                head: [
                    ['Nombre Completo', 'DNI', 'Área', 'Estado', 'Avance Checklist']
                ],
                body: employeeData,
                headStyles: {
            ...
            if (action === 'view') {
                ...
                if (previewWindow) {
                    previewWindow.document.write(
                        ...
                        '<iframe src="' + doc.output('datauristring') + '" style="width:100%; height:100%; border:none;"></iframe>' +
                        '</body></html>'
                    );
    ...
    function generateAndActionPdf(reportId, action) {
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text('REPORTE DE MANTENIMIENTO', pageWidth / 2, 20, {
            align: 'center'
        });
        ...
        const photosWithTitles = [{
            data: report.photo1,
            title: report.photoTitle1
        }, {
            data: report.photo2,
            title: report.photoTitle2
        }, {
            data: report.photo3,
            title: report.photoTitle3
        }].filter(p => p.data);
            ...
            photosWithTitles.forEach((photoItem, index) => {
                ...
                try {
                    if (photoItem.title) {
                        ...
                        doc.setTextColor(80);
                        doc.text(photoItem.title, xPos + imageWidth / 2, yPosTitle, {
                            align: 'center',
                            maxWidth: imageWidth
                        });
                    }
                    ...
                    doc.text('[Error]', xPos + (imageWidth / 2), yPosImage + (imageHeight / 2), {
                        align: 'center'
                    });
                } finally {
                    doc.setFont('helvetica', 'bold');
    ...
    // BLOQUE ACTUALIZADO: Función de bienvenida visual personalizada por nombre
    function showWelcomeMessage(userObject) {
        ...
        // Comprueba si el nombre del usuario está en nuestra lista
        ...
        messageBox.innerHTML = `
            <h1>${saludoVisual}, ${userName}! 👋</h1>
            <p>Te damos la bienvenida a Corporación Peruana de Suministros Logísticos SAC</p>
        `;
...
${fotoTexto}`;
    ...
    function generatePurchaseRequestPdf() {
        try {
            ...
            // 1. Título principal con tamaño de fuente reducido
            doc.setFontSize(16); // Reducido de 18 a 16
            doc.setFont('helvetica', 'bold');
            doc.text('FORMATO DE REQUERIMIENTO DE COMPRA', pageWidth / 2, currentY, { align: 'center' });

            // 2. Añadimos un pequeño espacio vertical para separar los textos
            currentY += 6; // Este es el espacio de separación

            // 3. Número de correlativo, ahora posicionado más abajo
            doc.setFontSize(12);
            doc.setFont('helvetica', 'normal');
            doc.text(nroCorrelativo, pageWidth - margin, currentY, { align: 'right' });
            
            // Movemos la posición para el siguiente bloque de contenido
            currentY += 10;
            ...
            const head = [['Item', 'Cant.', 'Und.', 'Especificación Técnica', 'Equipo/Área', 'Marca Sug.', 'Modelo/Parte']];
            const body = [];
            ...
            rows.forEach(row => {
                const rowData = [
                    ...
                    row.querySelector('[name="unidad"]').value,
                    row.querySelector('[name="especificacion"]').value,
                    row.querySelector('[name="equipo_uso"]').value,
                    row.querySelector('[name="marca"]').value,
                    row.querySelector('[name="modelo"]').value
                ];
                body.push(rowData);
            });
            ...
            doc.autoTable({
                startY: currentY,
                head: [['OBSERVACIONES / JUSTIFICACIÓN DE LA COMPRA']],
                body: [[observaciones]],
                theme: 'grid',
            ...
            // --- GUARDAR PDF ---
            doc.save(`Requerimiento_Compra_${nroCorrelativo.replace('N°: ', '')}.pdf`);
            displayMessage('PDF del requerimiento de compra generado con éxito.', 'success');
        
        } catch (error) {
            console.error("Error al generar el PDF de compra:", error);
            displayMessage('Ocurrió un error al generar el PDF.', 'error');
        }
    }
    ...
    window.actualizarColorPrioridadCompra = () => {
        ...
        select.className = 'w-full p-2 border border-gray-300 rounded-md shadow-sm font-bold'; // Reset
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            ...
            if (purchaseRequestModal.style.display === 'flex') purchaseRequestModal.style.display = 'none';
    ...
    function createAttentionDistributionChart(reports) {
        ...
        const layout = {
            title: 'Distribución del Tiempo de Atención (en días)',
            xaxis: {
    ...
    /**
     * Función para animar un texto con un efecto de máquina de escribir en bucle infinito.
     * @param {HTMLElement} element - El elemento HTML donde se mostrará el texto.
     * @param {string} text - El texto completo que se va a animar.
     * @param {number} speed - La velocidad de escritura en milisegundos.
     */
        ...
        const typeWriter = () => {
            ...
            if (isErasing) {
                ...
                if (i > 0) {
                    ...
                    i--;
                    setTimeout(typeWriter, speed / 2); // Borra un poco más rápido
                } else {
                    isErasing = false;
                    setTimeout(typeWriter, 500); // Pausa antes de volver a escribir
                }
            } else {
                // Fase de escritura
                if (i < text.length) {
                    ...
                    isErasing = true;
                    setTimeout(typeWriter, 2000); // Pausa de 2 segundos al final
                }
            }
        };

        typeWriter(); // Inicia el ciclo
    }
});